...
export const MatchesList = () => {
  ...
  const [players] = useQuery(getPlayers, { useGoalies: false }, { enabled: quizQuestions?.length > 0 });
  const [goalies] = useQuery(getPlayers, { useGoalies: true }, { enabled: quizQuestions?.length > 0 });

  const getAnswer = (answerId?: string | null) => teams?.find(team => team.id === answerId)?.name || players?.find(player => player.id === answerId)?.name || goalies?.find(g => g.id === answerId)?.name || t("UNANSWERED") as string;
            ...
            {quizQuestions.map((question) => {
              ...
              return (
                <Box key={question.id}>
                  ...
                  <Input disabled value={getAnswer(question.UserQuizQuestion.find(
